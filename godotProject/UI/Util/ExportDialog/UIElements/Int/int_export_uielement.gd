class_name int_export_uielement extends Control
...
static func make(v : Dictionary, input_target : Node) -> Control:
	var result = preload("res://UI/Util/ExportDialog/UIElements/Int/int_export_uielement.tscn").instantiate() as int_export_uielement
	result.title = v.name
...
func _on_val_value_changed(value: int) -> void:
	content = value
	update_target()
